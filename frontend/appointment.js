...
function generateDates() {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        if (isWeekend) {
            div.className = 'date-btn text-neutral-700 cursor-not-allowed flex items-center justify-center opacity-30 py-2';
            div.textContent = d;
...
function generateTimeSlots() {
    ...
    while (currentMin + interval <= endMin) {
        ...
        btn.type = 'button';
        btn.className = 'time-btn py-3 rounded-xl border border-neutral-800 text-xs font-medium hover:border-accent/50 transition text-neutral-400 bg-cardbg';
    ...
    timeBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            timeBtns.forEach(t => {
                ...
                t.classList.remove('bg-gradient-to-b', 'from-accent-light', 'via-accent', 'to-accent-dark', 'text-white', 'font-bold', 'active-time');
            ...
            btn.classList.add('bg-gradient-to-b', 'from-accent-light', 'via-accent', 'to-accent-dark', 'text-white', 'font-bold', 'active-time');
...
async function checkAvailableTimes(selectedDate) {
    ...
    try {
        const response = await fetch(`https://primecutsdb.onrender.com/api/appointments?date=${selectedDate}`)
        ...
        if (result.success) {
            ...
            timeBtns.forEach(btn => {
                ...
                if (bookedTimes.includes(btnTime)) {
                    btn.disabled = true;
                    btn.classList.remove('hover:border-accent/50', 'bg-gradient-to-b', 'from-accent-light', 'via-accent', 'to-accent-dark', 'text-white', 'font-bold', 'active-time', 'border-neutral-800', 'text-neutral-400', 'bg-cardbg');
...
openCheckoutBtn.addEventListener('click', () => {
    ...
    document.getElementById('modalPrice').textContent = `€${state.basePrice + state.addonPrice}`;
...
bookingForm.addEventListener('submit', async (e) => {
    ...
    const checkedAddonBoxes = document.querySelectorAll('.addon-checkbox:checked');
    const verifiedBasePrice = selectedCard ? parseInt(selectedCard.getAttribute('data-price')) : NaN;
    const verifiedAddonPrice = Array.from(checkedAddonBoxes).reduce((sum, box) => sum + parseInt(box.getAttribute('data-price')), 0);
    const verifiedTotal = verifiedBasePrice + verifiedAddonPrice;
    const modalDisplayedTotal = parseInt(document.getElementById('modalPrice').textContent.replace('€', ''));

    if (!Number.isFinite(verifiedTotal) || verifiedTotal !== (state.basePrice + state.addonPrice) || verifiedTotal !== modalDisplayedTotal) {
        errorMessage.textContent = 'Prijs kon niet worden geverifieerd. Vernieuw de pagina en probeer opnieuw.';
    ...
    try {
        const response = await fetch('https://primecutsdb.onrender.com/api/appointments', {
            method: 'POST',
        ...
        // PAYMENT INTEGRATION: once the backend returns data.paymentUrl instead of confirming instantly, redirect here (window.location.href = data.paymentUrl) instead of showing successContent =====
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (requestedService) {
        const targetCard = Array.from(serviceCards).find(card =>
            card.getAttribute('data-service').toLowerCase() === requestedService.toLowerCase()
        );
